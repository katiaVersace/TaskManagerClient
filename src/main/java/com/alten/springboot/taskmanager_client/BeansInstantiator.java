...
public class BeansInstantiator {
	...
	public ITeamController getTeamClient() {
		ITeamController proxy = JAXRSClientFactory.create("http://localhost:8080/resteasy", ITeamController.class,Collections.singletonList(new JacksonJaxbJsonProvider()));
		 WebClient.client( proxy ).accept( MediaType.APPLICATION_JSON );
		 WebClient.getConfig(proxy).getRequestContext().put(
			        org.apache.cxf.message.Message.MAINTAIN_SESSION, Boolean.TRUE);
